...
def pix2stamp(x, y, n):
    ...
    if n % 2:
        return tuple(
            slice(int(round(i)) - (n - 1) / 2, int(round(i)) + (n + 1) / 2)
            for i in (x, y)
        )
    else:
...
def semihex(data, axis=None):
    ...
    """
    h1, h5 = np.percentile(data, (100 / 6.0, 500 / 6.0), axis=axis)
    return (h5 - h1) / 2.0
...
def sault_weight(data, beam, sigma, correct=False):
    ...
    d = np.sum(data * beam / sigma**2, axis=0) / np.sum(
        beam**2 / sigma**2, axis=0
    )
    if correct is True:
        return d
...
class ImageStack(object):
    def __init__(self, h5filename, image_type="image", freq=None, steps=None, mode="r"):
        ...
        assert image_type in ("image", "dirty", "model", "moment"), (
            "Unsupported image_type %s." % image_type
        )
    ...
    def check_slice(self, x, y, margin=0):
        ...
        x_slice, y_slice = pix2stamp(
            x, y, margin + int(round(10 * max(1, sqrt_beamsize)))
        )
    ...
    def get_intervals(self):
        ...
        """
        return self.group.attrs["TIME_INTERVAL"] * np.arange(
    ...
    def world2pix(self, ra, dec, floor=True):
        ...
        if floor:
            pixcoord = np.round(pixcoord).astype(int)
    ...
    def scale_beam(self, beam):
        """
        NB scale should be 1D, but if it has multiple values these must be 
        for the polarisation (axis 0)
        """
        scale = np.squeeze(self.get_scale())
        if len(beam.shape)>len(scale.shape):
            scale = scale.reshape([*scale.shape]+[1]*(len(beam.shape)-len(scale.shape)))
        return beam * scale
    ...
    def pix2ts(self, x, y, avg_pol=True, correct=True):
        ...
        """
        ts = self.data[:, y, x, self.channel, self.steps[0] : self.steps[1]].astype(
            np.float_
        )
        if avg_pol is True:
    ...
    def slice2cube(self, x_slice, y_slice, avg_pol=True, correct=True):
        ...
        """
        ts = self.data[
            :, y_slice, x_slice, self.channel, self.steps[0] : self.steps[1]
        ].astype(np.float_)
    ...
    def pix2rms(self, x, y, avg_pol=True, correct=True):
        ...
        cube = self.pix2cube(
            x,
            y,
            int(round(10 * max(1, sqrt_beamsize))),
            avg_pol=avg_pol,
            correct=correct,
        )
        if avg_pol is True:
            ...
            return semihex(
                cube.reshape(cube.shape[0] * cube.shape[1], cube.shape[2]), axis=0
            )
        else:
            # cube has axes pol, x, y, time
            return semihex(
                cube.reshape(
                    cube.shape[0], cube.shape[1] * cube.shape[2], cube.shape[3]
                ),
                axis=1,
            )
