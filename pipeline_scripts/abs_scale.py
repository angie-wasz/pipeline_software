...
if __name__ == '__main__':
    ...
    parser.add_option("--csvfile", type="string", dest="csvfile", default=None, help="append to csv file")
    ...
    if opts.csv:
        if opts.csvfile is None:
            ...
            csvfile=open(opts.csvfile, 'a')
        print(f"{obsid}", end='', file=csvfile)
        for p in range(len(pols)):
            print(f",{scales[p]},{ns[p]},{frac_errors[p]}", end='', file=csvfile)
        print(file=csvfile)
