...
$(OBSID)_infield_$(SUFFIX).vot : $(HDF5)
	...
	#$(PYTHON) $(PIPELINE)/make_obs_cat.py $(HDF5) $(CAT_PATH)/ips_gleam_vlssr_dr2_230808.fits $(OBSID)_infield_$(SUFFIX).vot --ra_col=$(RA) --dec_col=$(DE)



#$(OBSID)_infield_$(SUFFIX).vot : $(OBSID).hdf5 
#	$(PYTHON) $(PIPELINE)/make_obs_cat.py $(OBSID).hdf5 /data/ips/catalogs/ips_gleam_vlssr_dr2_230808.fits $(OBSID)_infield_$(SUFFIX).vot --ra_col=$(RA) --dec_col=$(DE)
