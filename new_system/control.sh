...
# NEW SYSTEM
# The calibration and imaging pipeline are no longer separated and are now a single entity
# There will be the option to run one or the other, but the default will be to run both consecutively (for now)
...
cat ${OBSIDS} | xargs -P $NUM_PARALLEL -d $'\n' -n 1 bash ./run.sh --safemode ${SAFEMODE} -l ${LOG} -o 
